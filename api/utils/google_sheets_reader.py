...
def upload_ai_questions_from_sheet(spreadsheet_id, range_name):
    sheet_data = get_sheet_data(spreadsheet_id, range_name)
    ...
    if sheet_data:
        for row in sheet_data[1:]:
            ...
            correct_answer = row[8] if len(row) > 8 else ''
            difficulty = float(row[10]) if len(row) > 10 else 1.0
            # discrimination = float(row[11]) if len(row) > 11 else 1.0
            # guessing = float(row[12]) if len(row) > 12 else 0.25

            category_name = "Basic Theory"
